...
$qnpsn=array(':npsn'=>$_GET["npsn"]);
if(!isset($_SESSION["adm_id"]) || ($_SESSION["adm_level"]==2 && (!in_array(sql_value($connect, "SELECT dt_dati2 FROM tbl_sekdb WHERE dt_npsn=:npsn",$qnpsn),$_SESSION["adm_dati2"]) || !in_array(sql_value($connect, "SELECT dt_jp FROM tbl_sekdb WHERE dt_npsn=:npsn",$qnpsn),$_SESSION["adm_jp"])))){
include('../../403.html');
...
?>
<style>
#tblp td,#tblp th{border:1px solid;padding:.15rem;width:1rem;text-align:center;min-height:1.875rem;vertical-align:middle}@page{margin:1cm;}body{margin:0;font-family:Helvetica}#tblp{border-collapse:collapse;width:100%;font-size:10pt}h1,h2,h3{margin:0}h1{font-size:18pt}h2{font-size:14pt}h3{font-size:12pt}#tblp td:nth-child(2){text-align:left}#tblp td:last-child{background:none!important}#tblp th:last-child{background:#ddd!important}.nowrap{white-space: nowrap;}.nopb{page-break-inside:avoid!important}
...
<div style="text-align:center;margin-bottom:1rem">
  ...
  <div style="margin-left:50px">
  	...
  	<h1><?=sql_value($connect,"SELECT dt_nama FROM tbl_sekdb WHERE dt_npsn='".$_GET["npsn"]."'");?></h1>
  </div>
  </div>
  ...
  <tbody>
    ...
    function k_clean($x){
      ...
      if($x=='CT')return'C';
		...
		$q_gtk=sql_valarray($connect,"SELECT distinct(gtk_id) FROM tbl_dft_hadir WHERE dfhd_npsn='".$_GET["npsn"]."' AND (dfhd_tgl BETWEEN '".$_GET["t"].'-'.$_GET["b"].'-01'."' AND '".$_GET["t"].'-'.$_GET["b"].'-31'."')");
    if(!empty($q_gtk))$query = "
		SELECT gtk_id,gtk_no,gtk_nama,gtk_nip,statpeg FROM tbl_gtk
		WHERE gtk_npsn = '".$_GET["npsn"]."'
...
<div style="margin-top:12pt;margin-left:60%;text-align:center;font-size:10pt;" class="nopb">
  <?=sql_value($connect,"SELECT dt_dati3 FROM tbl_sekdb WHERE dt_npsn='".$_GET["npsn"]."'").", 1 ".bulan(date(m,strtotime($tb." +1 month")))." ".$_GET["t"];?><br/>
  Kepala Sekolah,
  <div style="margin-top:3rem"><b><u><?=sql_value($connect,"SELECT gtk_nama FROM tbl_gtk WHERE JSON_UNQUOTE(JSON_EXTRACT(gtk_jenis,'$[0]'))='Kepsek' AND gtk_npsn='".$_GET["npsn"]."'");?></u></b></div>
  <div>NIP. <?=sql_value($connect,"SELECT gtk_nip FROM tbl_gtk WHERE JSON_UNQUOTE(JSON_EXTRACT(gtk_jenis,'$[0]'))='Kepsek' AND gtk_npsn='".$_GET["npsn"]."'");?></div>
</div>
...
$dompdf->render();
$dompdf->stream("SISTER_RDH (".$_GET["npsn"].' - '.bulan($_GET["b"])." ".$_GET["t"].") (per ".date("Y-m-d H.i.s").").pdf");
