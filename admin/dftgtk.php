<?php
include('header.php');
$_GET["npsn"] = addslashes($_GET["npsn"]); $dt_npsn=array(':npsn'=>$_GET["npsn"]);
$dt2=sql_value($connect, "SELECT dt_dati2 FROM tbl_sekdb WHERE dt_npsn=:npsn",$dt_npsn);
$jp =sql_value($connect, "SELECT dt_jp FROM tbl_sekdb WHERE dt_npsn=:npsn",$dt_npsn);
$dbav=sql_value($connect, "SELECT sek_npsn FROM tbl_sekolah WHERE sek_npsn=:npsn",$dt_npsn);
if(empty($_GET["npsn"]) || $dbav==0 || $_SESSION["adm_level"]==3 || ($_SESSION["adm_level"]==2 && (!in_array($dt2,$_SESSION["adm_dati2"]) || !in_array($jp,$_SESSION["adm_jp"])))){?>
...
<div class="pb-2 mb-2 border-bottom d-block d-md-flex justify-content-between align-items-end">
	<h2 class="m-0"><div style="font-size:.875rem" class="text-muted"><?=$_GET["npsn"];?> / <?=sql_value($connect, "SELECT dt_nama FROM tbl_sekdb WHERE dt_npsn=:npsn",$dt_npsn);?></div>Daftar GTK</h2>
	<form class="d-flex align-items-center" autocomplete="off" method="get">
		<label class="mb-0 mr-2">NPSN:</label>
...
<form method="post" id="sek_lb_form">
	<div class="form-row">
		...
		<select class="form-control custom-select" name="lb_periode[]" <?if($_SESSION["adm_level"]!=0)echo'disabled';?> id="sek_lb_bulan">
		...
		</div><div class="col-sm-3 col-4 mb-3">
		<select class="form-control custom-select" name="lb_periode[]" <?if($_SESSION["adm_level"]!=0)echo'disabled';?> id="sek_lb_tahun">
			<option value="<?=date("Y");?>"><?=date("Y");?></option>
			<option value="<?=date("Y")-1;?>"><?=date("Y")-1;?></option>
		</select>
		<input type="hidden" name="action" value="update_lb"/> 
		<input type="hidden" name="npsn" id="lapbul_npsn"/>
		</div><div class="col-sm-4 mb-3 btn-group">
      <?if($_SESSION["adm_level"]==0){?>
      <input type="submit" class="btn btn-primary text-truncate" id="update_lb" value="Perbarui"/>
      <?}?>
      <button type="button" id="add_button" class="btn btn-success">Tambah GTK</button>
		</div>
	</div>
</form>
<table class="table table-bordered table-hover onload_ajax" style="min-width:2000px" id="tbl_dft_guru">
	<thead class="bg-light">
		<tr>
			...
			<th style="min-width:145px" rowspan="2">Tindakan</th>
		</tr>
		<tr>
			...
			<div class="modal-content">
				...
				<div class="modal-body row">
					<div class="col-lg-6">
						...
						<div class="fg d-flex justify-content-between">
					...
					<div class="col-lg-6">
						...
						<div class="fg" style="display:flex" id="pen_bantuan">
						...
						<div class="fg justify-content-between gaji_input" style="display:flex">
				...
				<div class="modal-footer">
					<input type="hidden" name="gtk_id" id="gtk_id" />
					<input type="hidden" name="npsn" id="form_npsn"/>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="border-top border-bottom py-2" id="modal_hps_nama"></div>
...
$jd1 = json_decode(sql_value($connect, "SELECT sek_lb_periode AS r FROM tbl_sekolah WHERE sek_npsn='".$_GET["npsn"]."'"));?>
...
$('#sek_lb_bulan').val('<?=$jd1[0];?>'); $('#sek_lb_tahun').val('<?=$jd1[1];?>');
$('[name=npsn],#form_npsn,#lapbul_npsn').val('<?=$_GET['npsn']?>');
$(document).ready(function(){
	var dataTable = $('#tbl_dft_guru').DataTable({
		...
		"ajax":{
			...
			type:"POST",
			data:{action:'fetch', type:"gtksek", npsn:'<?=$_GET["npsn"];?>'},
			dataType:"json",
...
<?}?>
	...
	$(document).on('click', '.btn_ubah', function(){
		...
		$.ajax({
			url:"action/gtk_single.php",
			method:"POST",
			data:{action:'edit_fetch', gtk_id:gtk_id, npsn:'<?=$_GET["npsn"];?>'},
			dataType:"json",
			success:function(x)
			{
				$('#spinner_loader').fadeOut();
	...
	$('#conf_hapus').click(function(){
		...
		$.ajax({
			url:"action/gtk_single.php",
			method:"POST",
			data:{gtk_id:gtk_id, action:"delete", gtk_nama:data_gtk_nama, npsn:'<?=$_GET["npsn"];?>'},
			dataType:"json",
			success:function(data)
			{
				if(data.success) topbar(1,data.text);
