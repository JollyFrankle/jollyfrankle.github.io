...
if(isset($_POST["gtk_id"]) && $_POST["action"]!='Tambah' && sql_value($connect, "SELECT count(gtk_id) FROM tbl_gtk WHERE gtk_id=:gtk AND gtk_npsn=:npsn",array(':gtk'=>$_POST["gtk_id"],':npsn'=>$_SESSION["sek_npsn"]))!=1) {
	die();
}
...
http_response_code(403);
	...
	// TINDAKAN: Menambah atau mengubah data GTK
	if($_POST["action"] == 'Tambah' || $_POST["action"] == 'Ubah')
	{
		if	(empty($_POST["gtk_sk_ber"]))
			$gtk_sk_ber = NULL; else
			$gtk_sk_ber = $_POST["gtk_sk_ber"];
		if($_POST["gtk_jenis"] == 'Kepsek' && sql_value($connect, "SELECT count(gtk_id) FROM tbl_gtk WHERE gtk_npsn = '".$_SESSION["sek_npsn"]."' AND gtk_id <> '".$_POST["gtk_id"]."' AND gtk_jenis = 'Kepsek'") > 0){
		...
		else
		{
			...
			// TINDAKAN: Menambah GTK baru
			if($_POST["action"] == 'Tambah')
			{
				$data[":gtk_npsn"]=$_SESSION["sek_npsn"];
				$query = "
					INSERT INTO tbl_gtk 
				...
				";
				sql_dbop($connect, "UPDATE tbl_sekolah SET sek_lastmod = '".date("Y-m-d H:i:s")."' WHERE sek_npsn='".$_SESSION["sek_npsn"]."'");
				$st = $connect->prepare($query);
				if($st->execute($data))
				{
					$opt = array(
						'success'	=>	true,
						"text"		=>	date("H:i:s").': GTK <b>'.filter_xsshtml($_POST["gtk_nama"]).'</b> telah ditambahkan untuk '.$_SESSION["sek_nama"]
					);
				}
			}
			
			// TINDAKAN: Mengubah data GTK
			if($_POST["action"] == 'Ubah')
			{
				...
				if($st->execute($data))
				{
					$opt = array(
						'success' =>	true,
						"text"		=>	date("H:i:s").': Data GTK <b>'.filter_xsshtml($_POST["gtk_nama"]).'</b> telah diubah untuk '.$_SESSION["sek_nama"]
					);
				}
			}
		}
	...
	// gtk_aktif: 1=Aktif (berarti bisa dipresensi), 2=Sementara direview, 3=Dihapus oleh sekolah (NOT YET IMPLEMENTED)
	...
	if($_POST["action"] == "delete")
	{
		...
		if($st->rowCount()==1)
		{
			$opt = array(
				'success' =>	true,
				"text"		=>	date("H:i:s").': Data GTK <b>'.filter_xsshtml($_POST["gtk_nama"]).'</b> telah dipindahkan ke daftar penghapusan.'
			);
		}
		else
		{
