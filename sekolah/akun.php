...
<form class="container m-auto px-0" method="post" autocomplete="off" id="form_akun" style="max-width:480px">
	...
	<div class="row">
		...
		<div class="col-8">
				<input class="form-control" id="curr_email" readonly value="<?php echo $_SESSION["sek_email"];?>" />
		</div>
		...
		<label class="col-4" for="sek_email">
			Alamat email
		</label>
		<div class="col-8">
			<input autocomplete="off" class="form-control" type="text" name="sek_email" id="sek_email" value="<?php echo $_SESSION["sek_email"];?>"/>
		</div>
		...
		<label class="col-4" for="sek_password">
			Kata sandi
		</label>
		<div class="col-8">
			<input class="form-control" type="password" name="sek_password" id="sek_password" value=""/>
		</div>
...
$(document).ready(function(){
	$('#sek_password, #p_conf').val('');

$('#form_akun').on('submit', function(event){
	event.preventDefault();
	if($('#sek_email').val()=='') $('#error_email').text('Alamat email/surel tidak boleh dibiarkan kosong!');
	else $('#error_email').text('');
	if($('#sek_password').val()=='') $('#error_password').text('Kata sandi tidak boleh dibiarkan kosong!');
	else $('#error_password').text('');
	if($('#sek_password').val()!=$('#p_conf').val()) $('#error_passconf').text('Kata sandi tidak sama!');
	else $('#error_passconf').text('');
	if($('#sek_password').val().length<8) $('#error_password').text('Kata sandi harus berisikan minimal 8 karakter.');
	else $('#error_password').text('');
